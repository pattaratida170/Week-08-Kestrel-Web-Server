...
1. ใน ESP-IDF v6.x ฟังก์ชัน `adc_oneshot_read()` คืนค่าผลลัพธ์เป็นตัวเลขแบบใด และมีช่วงค่าต่ำสุด-สูงสุดเท่าใดเมื่อตั้งค่าเป็น `ADC_BITWIDTH_12`
   - **คำตอบ:** คืนค่าเป็น จำนวนเต็ม (int) ของค่า ADC แบบดิจิทัล โดยเมื่อใช้ ADC_BITWIDTH_12 จะมีค่าตั้งแต่ 0–4095
1. ทำไมเราจึงต้องกดปุ่ม `Ctrl + ]` เพื่อปิด **ESP-IDF Monitor** ก่อนที่เราจะรันโปรแกรมฝั่ง C# Kestrel ในใบงานถัดไป
   *(คำใบ้: หากโปรเซส idf.py monitor ยังเปิดค้างอยู่ จะเกิดอะไรขึ้นกับพอร์ต COM)*
   - **คำตอบ:** เพราะ ESP-IDF Monitor จะจับใช้งาน พอร์ต Serial/COM อยู่ หากยังเปิดค้าง โปรแกรม C# Kestrel หรือโปรแกรมอื่นจะไม่สามารถใช้พอร์ตเดียวกันได้
---
...
- ทดลองใช้ไฟฉายจากโทรศัพท์มือถือส่อง และเอามือปิดบังแสง สังเกตค่า ADC บนหน้าจอเทอร์มินัลว่าเปลี่ยนแปลงอย่างไร
  เมื่อใช้ LDR แทนตัวต้านทานปรับค่าได้ ค่า ADC จะเปลี่ยนตามปริมาณแสง โดยเมื่อส่องไฟฉาย ค่า ADC จะเปลี่ยนไป และเมื่อใช้มือบังแสง ค่า ADC จะเปลี่ยนกลับตามระดับความสว่างของแสง
- บันทึกภาพถ่ายการต่อวงจรและภาพหน้าจอ Monitor ลงในรายงานผลการทดลอง
<img width="2940" height="1912" alt="image" src="https://github.com/user-attachments/assets/57aad42b-3efe-427e-9701-6659b0be6a84" />

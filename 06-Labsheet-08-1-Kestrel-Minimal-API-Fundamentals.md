...
2. กลับไปที่หน้าเบราว์เซอร์แล้วกดปุ่ม **Refresh (F5)** สังเกตว่าเกิดอะไรขึ้น และอธิบายสั้นๆ ว่าทำไมจึงเป็นเช่นนั้น
   - **คำตอบ** เมื่อกด Refresh หน้าเว็บไม่สามารถเข้าถึงได้และขึ้น ERR_CONNECTION_REFUSED เพราะ Kestrel Web Server หยุดทำงานอยู่ ทำให้เบราว์เซอร์ไม่สามารถเชื่อมต่อกับ localhost:5012 ได้
---
...
<img width="2940" height="1912" alt="image" src="https://github.com/user-attachments/assets/1359f817-5968-4e08-bf74-f919e47cf7ab" />

---

## คำถามท้ายการทดลอง (Review Questions)
1. ในสถาปัตยกรรมของ Kestrel ตัวแปร `builder` ทำหน้าที่อะไร และตัวแปร `app` ทำหน้าที่อะไร
	builder ทำหน้าที่เตรียมและตั้งค่าการทำงานของ Web Server เช่น Services และ Configuration ส่วน app ทำหน้าที่กำหนด Route หรือ Endpoint และรับคำขอจากผู้ใช้งาน		
2. เปรียบเทียบความสะดวกระหว่างการสร้าง Web Server บน .NET Minimal API กับการรันผ่าน LAMP Stack (Apache + PHP) ว่ามีข้อดีข้อเสียต่างกันอย่างไรในมุมมองของงาน IoT Gateway
	.NET Minimal API มีข้อดีคือเขียนโค้ดสั้น ตั้งค่าไม่ซับซ้อน และเหมาะกับการทำ API สำหรับ IoT โดยตรง แต่ต้องติดตั้ง .NET SDK ส่วน LAMP Stack ต้องติดตั้งและตั้งค่าหลายส่วน เช่น Apache และ PHP ทำให้เริ่มต้นยุ่งยากกว่า แต่เหมาะกับการทำเว็บไซต์และระบบที่ใช้ PHP
3. นักศึกษาคิดว่าการเพิ่ม `/api/` เข้าไปใน route นั้นมีประโยชน์อย่างไรบ้าง ถ้าไม่ใส่จะเกิดปัญหาอะไรบ้าง
	/api/ ช่วยแยก URL สำหรับการรับส่งข้อมูลของ API ออกจากหน้าเว็บไซต์ ทำให้จัดการ Route ได้ง่ายและเป็นระเบียบ หากไม่ใส่ก็ยังสามารถทำงานได้ แต่เมื่อมีหลาย Endpoint อาจทำให้ URL สับสนและจัดการระบบได้ยากขึ้น
